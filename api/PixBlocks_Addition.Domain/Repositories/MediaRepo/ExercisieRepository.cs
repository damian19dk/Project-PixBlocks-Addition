...
namespace PixBlocks_Addition.Domain.Repositories.MediaRepo
{
    public class ExercisieRepository : GenericRepository<Exercise>, IExerciseRepository
    {
        ...
        public async Task<IEnumerable<Exercise>> GetAsync(int page, int count = 10)
        {
            return await _entities.Exercises.Include(c => c.ExerciseVideos).ThenInclude(p => p.Video).ThenInclude(x => x.Tags)
